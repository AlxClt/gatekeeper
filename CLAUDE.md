...
- `evaluation/` is a separate concern with its own `requirements.txt` (pandas/datasets/sklearn/etc.) — only needed for rebuilding the eval dataset or running the notebooks, not for running the API itself. `evaluation/eval_dataset_clean.parquet` is excluded from version control; see `evaluation/create_eval_dataset.ipynb` for how it's built and `evaluation/results.md` for current model comparison numbers.
- Model choice tradeoffs (precision/recall/F1/FPR per model) live in the README and `evaluation/results.md` — check there before assuming a given local/online model is "good enough" for a change.
